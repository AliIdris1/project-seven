    ...
    <section className=' flexCenter flex-col overflow-hidden bg-feature-bg bg-center bg-no-repeat py-24'>
      ...
      <div className='z-20 flex w-full flex-col lg:w-[60%]'>
        <div className='relative'>
          ...
          <h2 className='bold-40 lg:bold-46'>Our Features</h2>
        </div>
        <ul className='mt-10 grid gap-10 md:grid-cols-2 lg:mt-20 lg:gap-20'>
          {FEATURES.map((F) =>  (
            <FeatureItem title={F.title} description={F.description} icon={F.icon} key={F.title} />
          ))}
        </ul>
      </div>
      </div>
...
const FeatureItem = ({title ,  description, icon }: FeatureItemts) => {
  return(
